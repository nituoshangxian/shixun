	...
	<div class="addEdit-block" :style='{"padding":"0 30px","margin":"0 auto","color":"#666","borderRadius":"4px","background":"none","width":"96%","fontSize":"16px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="申请编号" prop="shenqingbianhao">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="服务备注" prop="fuwubeizhu">
					<el-input v-model="ruleForm.fuwubeizhu" placeholder="服务备注" clearable  :readonly="ro.fuwubeizhu"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="服务备注" prop="fuwubeizhu">
					<el-input v-model="ruleForm.fuwubeizhu" placeholder="服务备注" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'" label="医生工号" prop="yishenggonghao">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.yishenggonghao" label="医生工号" prop="yishenggonghao">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="派单时间" prop="paidanshijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.paidanshijian" label="派单时间" prop="paidanshijian">
...
export default {
	...
	created() {
		this.ruleForm.paidanshijian = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='fuwubeizhu'){
							this.ruleForm.fuwubeizhu = obj[o];
							this.ro.fuwubeizhu = true;
							continue;
						}
						if(o=='yishenggonghao'){
            ...
            this.$http({
				url: `option/shequyisheng/yishenggonghao`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
			...
			yishenggonghaoChange () {
				this.$http({
					url: `follow/shequyisheng/yishenggonghao?columnValue=`+ this.ruleForm.yishenggonghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `fuwuzhipai/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fuwuzhipaiCrossAddOrUpdateFlag = false;
											this.parent.search();
