<template>
	<div class="main-content" :style='{"padding":"0px","margin":"0 auto","color":"#333","borderRadius":"0 0 10px 10px","background":"rgba(255,255,255,.0)","width":"96%","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px","margin":"20px 0 0","overflow":"hidden","color":"#666","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"1px solid #dcd6c3","padding":"10px 20px","margin":"0 30px 30px 0","alignItems":"center","borderRadius":"8px","flexWrap":"wrap","background":"rgba(251,249,243,.9)","display":"flex","width":"auto","fontSize":"18px","justifyContent":"center"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">咨询标题</label>
						<el-input v-model="searchForm.zixunbiaoti" placeholder="咨询标题" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"border":"1px solid #dcd6c3","padding":"10px 20px","margin":"0 0 30px","color":"#666","alignItems":"center","display":"flex","-webkitPerspective":"320px","justifyContent":"center","borderRadius":"8px","flexWrap":"wrap","flex":"1","background":"rgba(251,249,243,.9)","width":"auto","perspective":"320px","fontSize":"18px"}'>
					<el-button class="add" v-if="isAuth('yishengzixun','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('yishengzixun','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('yishengzixun','咨询科室类别')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						咨询科室类别
					</el-button>
					<el-button class="btn18" v-if="isAuth('yishengzixun','医生咨询次数')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						医生咨询次数
					</el-button>
				</el-row>
			...
			<div :style='{"padding":"0 30px","borderColor":"#eee","margin":"30px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.69)","borderWidth":"0px 0 0 0px","width":"100%","borderStyle":"solid"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('yishengzixun','审核')" prop="sfsh" label="回复">
						<template slot-scope="scope">
							<el-button  type="text" width="50% @click="shDialog(scope.row)">回复</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yishengzixun','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="edit" v-if=" isAuth('yishengzixun','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('yishengzixun','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yishengxingmingChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var keshiChart1 = echarts.init(document.getElementById("keshiChart1"),'cool');
        this.$http({
            url: "yishengzixun/group/keshi",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var yishengxingmingChart2 = echarts.init(document.getElementById("yishengxingmingChart2"),'cool');
        this.$http({
            url: "yishengzixun/group/yishengxingming",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                let pArray = []
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].yishengxingming);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].yishengxingming
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                yishengxingmingChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yishengxingmingChart2.resize();
                };
            }
        });
      })
	...
	yishengzixunstatusChange(e,row){
		if(row.status==0){
