<template>
	<div class="main-content" :style='{"padding":"0px","margin":"0 auto","color":"#333","borderRadius":"0 0 10px 10px","background":"rgba(255,255,255,.0)","width":"96%","fontSize":"16px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('jiankangxinxi','健康评测')" @click="jiankangpingceCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','此次健康申报已评估')" type="success">
		...
		<jiankangpingce-cross-add-or-update v-if="jiankangpingceCrossAddOrUpdateFlag" :parent="this" ref="jiankangpingceCrossaddOrUpdate"></jiankangpingce-cross-add-or-update>
...
import jiankangpingceCrossAddOrUpdate from "../jiankangpingce/add-or-update.vue";
	export default {
		data() {
			return {
				searchForm: {
				...
				jiankangpingceCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			jiankangpingceCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			jiankangpingceCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jiankangpingceCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jiankangpingceCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jiankangpingceCrossaddOrUpdate.init(row.id,type);
				});
			},
